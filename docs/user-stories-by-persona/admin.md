...
**Suppress an audit log.** Every admin action writes `AdminAuditLog`, including the actions of super-admins.
...
- **State:** Permission checked against `Role.permissions`; denial logged.
...
### US-15.11 · Have every action of mine audited `A` `S`
- **State:** `AdminAuditLog` — actor, entity, before, after, timestamp, IP.
- **Edge:** **Before-and-after values, not just the action name.** "Admin edited service" is useless; "changed price 499 → 299" is evidence.

### US-15.12 · Review the audit trail `A`
- **State:** Read of `AdminAuditLog`, filterable by actor, entity and window.
- **Edge:** Must be **searchable by entity**, so "who touched this booking?" is one query rather than a scan.

### US-15.13 · Change a role's permissions `A`
- **State:** `Role.permissions` updated; audited.
...
- **Edge:** The ticket is the only record of *why*.
